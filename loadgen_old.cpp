...
// Generate "value<N>"
string make_value(int n) {
    return "value" + to_string(n);
}

// Generate "key<N>"
string make_key(int n) {
    return "key" + to_string(n);
}
...
void client_thread_func(
    ...
    static vector<string> popular_keys = { "key1", "key2", "key3", "key4", "key5" };

    //mt19937 rng(id + time(NULL));
    long long valid = 0;
    uniform_int_distribution<int> key_dist(1, 500000);
    uniform_int_distribution<int> pop_dist(1, 5);
    uniform_int_distribution<int> mix_dist(0, 2);

    while (steady_clock::now() < end_time) {
        //int key_num = key_dist(rng);
        int key_num = (valid % 500000) + 1;
        valid++;
        string key = make_key(key_num);
        string value = make_value(key_num);
        ...
        switch (workload) {
            ...
            case POPULAR_GET: {
                //int idx = pop_dist(rng);
                int idx = (valid % 5) + 1;
                string hot_key = "key" + to_string(idx);
            ...
            case POPULAR_PUT: {
                ...
                int idx = (valid % 5) + 1;
                string hot_key = "key" + to_string(idx);
                string body = "key=" + hot_key + "&value=" + make_value(idx);
                auto res = cli.Post("/create", body, "application/x-www-form-urlencoded");
                success = (res && res->status == 200);
                break;
            }
...
Workload parse_workload(const string &s) {
    ...
    if (s == "popular_put") return POPULAR_PUT;
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        ...
        cout << "Workloads: putall | getall | popular | mixed | popular_put\n";
        return 1;
    }
